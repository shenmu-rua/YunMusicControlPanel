...
/*
 * http_get - Perform HTTP GET request, return response body as malloc'd string
 ...
 */
static int http_get_no_response(const wchar_t* host, int port, const wchar_t* path, int use_https) {
    HINTERNET hSession = NULL, hConnect = NULL, hRequest = NULL;
    int ret = -1;
...
int api_bot_poll_status(BotStatus* status) {
    ...
    /* Build merged status query path */
    /* /api/bot/use/0/(/json/merge/(/song)/(/volume)/(/repeat)/(/random)) */
    const wchar_t* status_path =
        L"/api/bot/use/0/(/json/merge/(/song)/(/volume)/(/repeat)/(/random))";

    char* response = http_get(host, g_base_port, status_path, g_use_https);
...
static int send_command(const char* cmd) {
    ...
    return http_get_no_response(host, g_base_port, path, g_use_https);
}
...
int api_bot_send_command_post(const char* command) {
    ...
    HINTERNET hSession = WinHttpOpen(L"YunMusicPlugin/1.0",
        WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!hSession) return -1;

    HINTERNET hConnect = WinHttpConnect(hSession, host, (INTERNET_PORT)g_base_port, 0);
